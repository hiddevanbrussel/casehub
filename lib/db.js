...
const { nowIso, includesQ, fullName, STATUSSEN, PRIORITEITEN, ROLLEN, labelOf, overviewGroup, progressOf, OVERVIEW_GROUPS } = require("./format");
...
function publicWerknemer(w) {
  ...
  return { ...rest, naam: fullName(w), rolLabel: labelOf(ROLLEN, w.rol) };
}
...
const store = {
  ...
  recentBijlagen(limit = 6) {
    return db
      ...
      .value()
      .map((b) => {
        const zaak = db.get("zaken").find({ id: b.zaakId }).value();
        return { ...b, zaaknummer: zaak?.zaaknummer, zaakTitel: zaak?.titel };
      });
  },
  auditLog(limit = 80) {
  ...
  werknemer(wid) {
    return publicWerknemer(db.get("werknemers").find({ id: wid }).value());
  },
  ...
  saveWerknemer(input, actor) {
    ...
    const record = {
      ...
      rol: input.rol || "medewerker",
      afdeling: input.afdeling?.trim() || "",
    ...
    if (!record.wachtwoord && record.authSource !== "azure") throw new Error("A password is required for a new employee.");
  ...
  addBijlage(zaakId, file, actor) {
    ...
    audit(actor, "bijlage", "zaak", zaakId, file.originalname);
    return record;
  },
  bijlage(bid) {
  ...
  search(q) {
    ...
    return {
      ...
      werknemers: store.werknemers().filter((w) => includesQ(`${w.naam} ${w.email}`, query)).slice(0, 6),
    };
  },
};
...
function migrateOnboarding() {
  ...
  if (db.get("werknemers").size().value() > 0) {
    db.get("meta")
      .assign({ onboarded: true, setupOrgDone: true, setupCreateDone: true, setupAzureDone: true })
      .write();
  }
}
